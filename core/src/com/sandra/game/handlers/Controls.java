...
public class Controls {
    ...
    public Controls(DelayedRemovalArray<Entity> entities) {this.entities = entities;}
    ...
    /*
        ...
        tilted towards their face, thus making it more comfortable to play.
    */
    public void android_movement(float delta, Entity entity) {  // Accelerometer  = x = y = [-10, 10]
        if (Gdx.input.getAccelerometerX() < android_tilt_sensitivity_up) {move_entity_up(delta, entity);}               // up
        else if (Gdx.input.getAccelerometerX() > android_tilt_sensitivity_down) {move_entity_down(delta, entity);}      // down
        else {entity.get_velocity().y = 0;}
        
        if (Gdx.input.getAccelerometerY() < android_tilt_sensitivity_left) {move_entity_left(delta, entity);}           // left
        else if (Gdx.input.getAccelerometerY() > android_tilt_sensitivity_right) {move_entity_right(delta, entity);}    // right
    ...
    public void desktop_movement(float delta, Entity entity) {
        if (Gdx.input.isKeyPressed(Keys.W)) {move_entity_up(delta, entity);}            // up
        else if (Gdx.input.isKeyPressed(Keys.S)) {move_entity_down(delta, entity);}     // down
        else {entity.get_velocity().y = 0;}
        
        if (Gdx.input.isKeyPressed(Keys.A)) {move_entity_left(delta, entity);}          // left
        else if(Gdx.input.isKeyPressed(Keys.D)) {move_entity_right(delta, entity);}     // right
        else {entity.get_velocity().x = 0;}
